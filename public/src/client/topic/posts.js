...
define("forum/topic/posts", [
	...
	Posts.onNewPost = function (data) {
		...
		data.posts[0].timestamp = data.posts[0].topic.scheduled
			? data.posts[0].timestamp
			: Date.now() - 1000;
	...
	Posts.modifyPostsByPrivileges = function (posts) {
		posts.forEach(function (post) {
			post.selfPost =
				!!app.user.uid && parseInt(post.uid, 10) === parseInt(app.user.uid, 10);
			post.topicOwnerPost =
				parseInt(post.uid, 10) === parseInt(ajaxify.data.uid, 10);

			post.display_edit_tools =
				(ajaxify.data.privileges["posts:edit"] && post.selfPost) ||
				ajaxify.data.privileges.isAdminOrMod;
			post.display_delete_tools =
				(ajaxify.data.privileges["posts:delete"] && post.selfPost) ||
				ajaxify.data.privileges.isAdminOrMod;
			...
			post.display_post_menu =
				...
				(post.selfPost &&
					post.deleted &&
					parseInt(post.deleterUid, 10) === parseInt(app.user.uid, 10)) ||
	...
	function updatePostCounts(posts) {
		for (let i = 0; i < posts.length; i += 1) {
			...
			cmp.html(
				helpers.formattedNumber(parseInt(cmp.attr("data-postcount"), 10) + 1),
			);
		}
	}
	...
	function onNewPostPagination(data) {
		...
		ajaxify.data.pagination.pageCount = Math.max(
			1,
			Math.ceil(posts[0].topic.postcount / config.postsPerPage),
		);
		const direction =
			config.topicPostSort === "oldest_to_newest" ||
			config.topicPostSort === "most_votes"
				? 1
				: -1;

		const isPostVisible =
			(ajaxify.data.pagination.currentPage ===
				ajaxify.data.pagination.pageCount &&
				direction === 1) ||
			(ajaxify.data.pagination.currentPage === 1 && direction === -1);

		if (isPostVisible) {
			const repliesSelector = $(
				'[component="post"]:not([data-index=0]), [component="topic/event"]',
			);
			createNewPosts(data, repliesSelector, direction, false, scrollToPost);
		} else if (
			ajaxify.data.scrollToMyPost &&
			parseInt(posts[0].uid, 10) === parseInt(app.user.uid, 10)
		) {
	...
	function onNewPostInfiniteScroll(data) {
		...
		const isPreviousPostAdded = $(
			'[component="post"][data-index="' + (data.posts[0].index - 1) + '"]',
		).length;
		if (
			!isPreviousPostAdded &&
			(!data.posts[0].selfPost || !ajaxify.data.scrollToMyPost)
		) {
			return;
		}
	...
	function createNewPosts(
		...
		function removeAlreadyAddedPosts() {
			...
			if (newPosts.length === data.posts.length) {
				...
				newPosts.each(function (index, el) {
					if (
						parseInt($(el).attr("data-pid"), 10) !==
						parseInt(data.posts[index].pid, 10)
					) {
						allSamePids = false;
					}
				});
			...
			data.posts = data.posts.filter(function (post) {
				return (
					post.allowDupe ||
					$('[component="post"][data-pid="' + post.pid + '"]').length === 0
				);
			});
		}
		...
		app.parseAndTranslate(
			"topic",
			"posts",
			Object.assign({}, ajaxify.data, data),
			async function (html) {
				html = html.filter(function () {
					...
					return (
						...
						allowDupe ||
						(pid &&
							$('[component="post"][data-pid="' + pid + '"]').length === 0)
					);
				});
			...
			if (btn) {
				btn.addEventListener("click", function () {
					...
					api.put(`/posts/${pid}/endorse`, { endorsed: true }, function (err) {
						...
						const message = post.querySelector(
							'[component="topic/post/endorse-message"]',
						);
						...
						const container = foundPost[1].querySelector(
							'[component="topic/post/endorse-message"]',
						);
						...
						const currBtn = foundPost[1].querySelector(
							'[component="topic/post/endorse"]',
						);
						if (currBtn) {
							currBtn.remove();
						}
	...
	Posts.loadMorePosts = function (direction) {
		...
		const replies = components
			.get("topic")
			.find(components.get("post").not("[data-index=0]").not(".new"));
	...
	Posts.onTopicPageLoad = async function (posts) {
		...
		posts
			.find('[component="post/content"] img:not(.not-responsive)')
			.addClass("img-fluid");
	...
	Posts.addTopicEvents = async function (events) {
		...
		if (config.topicPostSort === "oldest_to_newest") {
			...
			const mainPost = $(
				'[component="topic"] [component="post"][data-index="0"]',
			);
			if (mainPost.length) {
	...
	async function addNecroPostMessage() {
		...
		if (
			!necroThreshold ||
			(config.topicPostSort !== "newest_to_oldest" &&
				config.topicPostSort !== "oldest_to_newest")
		) {
			return;
		}
		...
		await Promise.all(
			postEls.map(async function (post) {
				post = $(post);
				...
				if (
					config.topicPostSort === "newest_to_oldest" &&
					parseInt(prev.attr("data-index"), 10) === 0
				) {
					return;
				}
				...
				if (Math.abs(diff) >= necroThreshold) {
					...
					const translationText =
						(diff > 0 ? "[[topic:timeago-later," : "[[topic:timeago-earlier,") +
				...
				$(postEl)
					...
					.each(function (idx, imgEl) {
						...
						if (
							imgEl
								.attr("src")
								.startsWith(config.relative_path + config.upload_url)
						) {
	...
	Posts.addBlockquoteEllipses = function (posts) {
		const rootBlockQuotes = posts
			.find('[component="post/content"] blockquote')
			.filter((i, el) => !$(el).parent().is("blockquote"));
		const nestedBlockQuote = rootBlockQuotes.find(">blockquote");
		nestedBlockQuote.each(function () {
			const $this = $(this);
			if (
				$this.find(":hidden:not(br)").length &&
				!$this.find(".toggle").length
			) {
				$this.append(
					'<i class="d-inline-block fa fa-angle-down pointer toggle py-1 px-3 border text-bg-light"></i>',
				);
			}
		});
	};
