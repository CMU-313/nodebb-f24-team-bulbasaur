...
define("forum/topic/replies", [
	...
	"alerts",
	"api",
], function (posts, hooks, alerts, api) {
	const Replies = {};
			...
			api.get(`/posts/${pid}/replies`, {}, function (err, { replies }) {
				...
				app.parseAndTranslate("topic", "posts", tplData, async function (html) {
					...
					if (button.attr("data-target-component")) {
						post
							.find(
								'[component="' + button.attr("data-target-component") + '"]',
							)
							.html(repliesEl);
					} else {
	...
	function incrementCount(post, inc) {
		...
		const count = Math.max(
			0,
			(parseInt(countEl.attr("data-replies"), 10) || 0) + inc,
		);
		...
		const hasSingleImmediateReply =
			count === 1 && Math.abs(post.index - index) === 1;
		...
		if (!avatars.find('[data-uid="' + post.uid + '"]').length && count < 7) {
			app.parseAndTranslate(
				...
				{
					posts: [
						{ replies: { count: count, hasMore: false, users: [post.user] } },
					],
				},
				function (html) {
					avatars.prepend(
						html.find('[component="post/reply-count/avatars"]').html(),
					);
				},
			);
		}
